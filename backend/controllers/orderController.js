...
const updateOrderStatus = asyncHandler(async (req, res, next) => {
  ...
  try {
    // 1. Handle stock rollback when cancelling order
    ...
    // 2. Handle stock adjustment if order is being un-cancelled
    ...
    // 3. Increment soldCount when order is completed
    ...
    return res.status(500).json({
      success: false,
      message: 'Cập nhật trạng thái thất bại: ' + err.message,
      data: null,
    });
  }
...
const cancelOrder = asyncHandler(async (req, res, next) => {
  ...
  try {
    // 1. Hoàn stock cho tất cả sản phẩm trong đơn
    for (const item of order.items) {
      await Product.updateOne(
        { _id: item.product },
        { $inc: { stock: item.quantity } },
    ...
    // 2. Hoàn coupon: giảm usageCount để coupon có thể sử dụng lại
    ...
    return res.status(500).json({
      success: false,
      message: 'Hủy đơn hàng thất bại: ' + err.message,
      data: null,
    });
  }
